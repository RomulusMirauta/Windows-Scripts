...
& ffmpeg -n -i $inputFile.FullName -vf "fps=15,scale=640:-1:flags=lanczos,palettegen" $palette
...
& ffmpeg -n -i $inputFile.FullName -i $palette -filter_complex "fps=15,scale=640:-1:flags=lanczos[x];[x][1:v]paletteuse" -loop 0 $output
if ($LASTEXITCODE -ne 0) {
    Write-Host ""
